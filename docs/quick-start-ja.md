...
bit-relay は `bit hub` のメタデータ（Issue, PR, Note 等）を中継するリレーサーバーです。 Git
オブジェクト（コミット、ブロブ等）は保持しません。

つまり、リポジトリの **コード** は従来どおり GitHub 等の Git リモートから clone し、 **hub
データ**（Issue や PR）を relay 経由で同期する、という二段構成になります。
...
`relay+https://` プレフィックスが relay モードを明示します。 省略して `https://...`
にすると、smart-http を先に試行し 404 時に relay fallback します。
...
- relay は hub メタデータ（Issue/PR/Note）のみを中継します。`bit clone` や `bit push` の Git
  オブジェクト転送には使えません
